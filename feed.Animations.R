...
exact.dist.plot <- function(max.count=30) {
  ...
  unique.dist <- function(max.count) {
    ...
    for (i in 1:max.count) {
      unique.list[[i]] <- mapply(function(x) sum(round(signif(spread.df[spread.df[, "sum_count"] == i, "Average"], 8) %in% signif(short.options[[i]][x], 8))), 1:length(short.options[[i]]))
    }
    return(unique.list)
  }
  avg.dist <- unique.dist(max.count)
  exact.movie <- function(max.count) {
    # Laziest hack ever to avoid writing a function to distinguish "Rating" from "Ratings"
